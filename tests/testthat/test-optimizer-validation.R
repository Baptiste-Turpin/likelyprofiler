...
test_that("Optimizer validation works correctly", {
  ...
  expect_error(
    ...
    "Error in validateAndSetupOptimizer: Unknown optimizer"
  )
  ...
  expect_error(
    ...
    "Error in validateAndSetupOptimizer: Custom optimizer must have signature"
  )
...
test_that("Bounds validation in validateAndSetupOptimizer works", {
  ...
  expect_error(
    ...
    "Error in validateAndSetupOptimizer: bounds must be a list with 'lower' and 'upper' elements"
  )

  # Test non-numeric bounds
  expect_error(
    validateAndSetupOptimizer("optim", list(), bounds = list(lower = "a", upper = "b")),
    "Error in validateAndSetupOptimizer: bounds\\$lower and bounds\\$upper must be numeric vectors"
  )

  # Test mismatched lengths
  expect_error(
    validateAndSetupOptimizer("optim", list(), bounds = list(lower = c(1, 2), upper = c(1))),
    "Error in validateAndSetupOptimizer: bounds\\$lower and bounds\\$upper must have the same length"
  )

  # Test lower >= upper
  expect_error(
    validateAndSetupOptimizer("optim", list(), bounds = list(lower = c(2), upper = c(1))),
    "Error in validateAndSetupOptimizer: bounds\\$lower must be less than bounds\\$upper for all elements"
  )
  ...
  "Warning in validateAndSetupOptimizer: Method 'Nelder-Mead' specified for optim with bounds. Changing to 'L-BFGS-B'"
  )
...
test_that("DEoptim optimizer works if available", {
  if (requireNamespace("DEoptim", quietly = TRUE)) {
    ...
    expect_error(
      ...
      "Error in validateAndSetupOptimizer: DEoptim package required but not available"
    )
  }
})
